...
void nms(customized_msgs::BboxList& res, float *output, float conf_thresh, float nms_thresh = 0.5) {
    int det_size = sizeof(Yolo::Detection) / sizeof(float);
    customized_msgs::Bbox bbox;
...
//            res.push_back(item);
            bbox.conf = item.conf;
            bbox.label = (int8_t)item.class_id;
            bbox.bbox[0]=item.bbox[0];
            bbox.bbox[1]=item.bbox[1];
            bbox.bbox[2]=item.bbox[2];
            bbox.bbox[3]=item.bbox[3];
            res.Bboxlist.push_back(bbox);
