...
ICudaEngine* build_engine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, std::string& wts_name) {
    ...
    auto conv33 = convBlock(network, weightMap, *deconv32->getOutput(0), 5, 3, 1, 1, "model.33");
    // segmentation output
    ISliceLayer *slicelayer = network->addSlice(*conv33->getOutput(0), Dims3{ 0, (Yolo::INPUT_H - Yolo::IMG_H) / 2, 0 }, Dims3{ 5, Yolo::IMG_H, Yolo::IMG_W }, Dims3{ 1, 1, 1 });
